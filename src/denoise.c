...
Description: Denoising using frequency gate and fft overlapping
...
// This function handles the FFT based gate denoising
void process_denoise(int16_t *inBuffer, int inWritePointer, int16_t *outBuffer, int outWritePointer)
{
...
{
    fprintf(stderr, "%s in.wav out.wav <threshold> <attenuation>\n", name);
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc - optind < 3)
    {
        printf("Warning: not enough parameter provided\n");
        printf("         using thresh = %d and atten = %f by default\n", thresh, atten);
    }
    ...
    if (argc - optind > 2)
    {
        thresh = atoi(argv[optind + 2]);
    }
    if (argc - optind > 3)
    {
        atten = atof(argv[optind + 3]);
    }
    ...
    for(n = 0; n < numSamples; n+=channels)
    {
        // Read audio inputs
        if(channels == 1)
        {
            inL = (convert_buf[n]);///(1<<15);
            inR = inL;
        }
        else if(channels == 2)
        {
...
#if 1 // apply denoising. otherwise it's bypassed
        ...
        if(gSampleCount >= Hs)
        {
            process_denoise(gInputBuffer, gInputBufferPointer, gOutputBuffer, gOutputBufferWritePointer);
            gSampleCount = 0;
        }    
